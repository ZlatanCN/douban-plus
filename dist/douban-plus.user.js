...
(function() {
	...
	var getXhr = () => {
		const g = GM_xmlhttpRequest === void 0 ? void 0 : GM_xmlhttpRequest;
	...
	var TTL_MS$2 = 1800 * 1e3;
	var STORAGE_KEY$2 = "dp:avatar-cache";
	var loadCache$2 = () => {
		try {
	...
	var persistCache$2 = (cache) => {
		try {
			localStorage.setItem(STORAGE_KEY$2, JSON.stringify([...cache.entries()]));
		} catch {}
	};
	var cache = loadCache$2();
	var cacheGet$2 = (link) => {
		const entry = cache.get(link);
		if (!entry) return;
		if (Date.now() > entry.expiresAt) {
			cache.delete(link);
			persistCache$2(cache);
			return;
		}
		return entry.url;
	};
	var cacheSet$2 = (link, url) => {
		cache.set(link, {
			expiresAt: Date.now() + TTL_MS$2,
			url
		});
		persistCache$2(cache);
	};
	...
	var resolveCommentAvatars = async (comments) => {
		const pending = comments.filter((c) => c.link && !cacheGet$2(c.link));
		for (const c of comments) {
			const url = cacheGet$2(c.link);
			if (!url) continue;
			c.avatar = url;
	...
	var TTL_MS$1 = 1440 * 60 * 1e3;
	var STORAGE_KEY$1 = "dp:imdb-cache-v2";
	var loadCache$1 = () => {
		try {
	...
	var cacheGet$1 = (id) => {
		const entries = loadCache$1();
		const entry = entries.get(id);
		if (!entry) return;
		if (Date.now() > entry.expiresAt) {
			entries.delete(id);
			persistCache$1(entries);
			return;
		}
		return {
			rating: entry.rating,
	...
	var cacheSet$1 = (id, rating, title) => {
		const entries = loadCache$1();
		entries.set(id, {
  ...
  title(id: $id) {
	...
	var fetchImdbRating = async (imdbId, season) => {
		...
		const cached = cacheGet$1(imdbId);
		...
		if (season && seriesIdForSeason) {
			...
			try {
				...
				if (seasonResult) {
					...
					cacheSet$1(sKey, seasonResult.rating, title);
					return {
		...
		if (typeof effectiveRating === "number" && typeof effectiveCount === "number") {
			...
			cacheSet$1(imdbId, rating, title);
			return {
				rating,
				title
			};
	...
	var STORAGE_KEY = "dp:rotten-cache";
	var loadCache = () => {
		try {
	...
	var cacheKey = (slug, season) => season ? `${slug}-s${String(season).padStart(2, "0")}` : slug;
	var parseData = (raw) => {
		const data = JSON.parse(raw);
		const criticsScore = data.criticsScore?.score;
		if (criticsScore === void 0 || criticsScore === null) return null;
		const score = typeof criticsScore === "string" ? Number.parseInt(criticsScore, 10) : criticsScore;
		if (Number.isNaN(score)) return null;
		return {
			count: data.audienceScore?.reviewCount ?? 0,
			score
		};
	};
	var parseMediaScorecard = (html) => {
		const mc = html.match(/<script[^>]*data-json="mediaScorecard"[^>]*>(?<json>.*?)<\/script>/iu);
	...
	var parseRtPage = (html) => {
		const match = html.match(/<script[^>]*type="application\/json"[^>]*data-json="reviewsData"[^>]*>(?<json>.*?)<\/script>/iu);
	...
	var fetchRtRating = async (title, isTV, season) => {
		...
		} else urls.push(`https://www.rottentomatoes.com/m/${slug}`);
	...
	var LOGO_RT = "<svg viewBox=\"0 0 44 18\" width=\"44\" height=\"18\" aria-label=\"Rotten Tomatoes\"><rect width=\"44\" height=\"18\" rx=\"3\" fill=\"#FA320A\"/><text x=\"22\" y=\"13\" text-anchor=\"middle\" fill=\"#fff\" font-family=\"-apple-system,BlinkMacSystemFont,Arial,Helvetica,sans-serif\" font-weight=\"800\" font-size=\"11\">RT</text></svg>";
	...
	var buildRtRating = (rating) => {
		...
		const detail = el("div", { className: "atv-rating-panel-detail" });
		const countTxt = rating.count ? `${rating.count.toLocaleString("en-US")} 评论` : "";
		if (countTxt) detail.append(el("div", {
			className: "atv-rating-panel-count",
			text: countTxt
		}));
		section.append(detail);
	...
	var extractSeasonFromH1 = (h1) => {
		...
		return m?.groups?.seasonDigit ? Number.parseInt(m.groups.seasonDigit, 10) : void 0;
	};
	var resolveRtRating = async (title, isTV, season) => {
	...
	var resolveImdbRating = async (imdbId, isTV, season) => {
		...
		if (rtTitle) resolveRtRating(rtTitle, isTV, rtSeason);
	};
