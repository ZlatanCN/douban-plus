...
`;
  ...
  title(id: $id) {
		...
		return u("div", {
			...
			children: [
				u("span", {
					...
					"aria-hidden": "true",
					children: Icon ? u(Icon, {}) : u(IconBroadcast, {})
				}),
				u("span", {
					class: "atv-first-broadcast-platform-label",
					children: "首播平台"
				}),
				u("span", {
					class: "atv-first-broadcast-platform-name",
					children: platform
				})
			]
		});
	...
	var HeroMeta = ({ hasFirstBroadcastPlatform = false, info, isTV, year }) => {
		const metaParts = [];
		...
		return u("div", {
			class: `atv-hero-meta${hasFirstBroadcastPlatform ? " has-first-broadcast-platform" : ""}`,
					...
					children: u("div", {
						...
						children: [u(HeroPoster, {
							...
							children: [
								...
								firstBroadcastPlatform ? u(FirstBroadcastPlatform, { platform: firstBroadcastPlatform }) : null,
								u(RatingPanel, {
									douban: data.rating,
									imdbId: data.imdbId,
									isTV: data.isTV
	...
	var mountIframe = (host, iframe, onError, isStopped) => {
	...
	var mountNativeLoginFrame = (host, onError) => {
		let stopped = false;
		let retryTimer;
		const stop = () => {
			stopped = true;
	...
	var DOUBAN_GROUP_ORIGIN = "https://www.douban.com/";
	var GROUP_TOPIC_PATH = /^\/group\/topic\/\d+\/?$/u;
	var GROUP_COLLECTION_PATH = /^\/group\/\d+\/?$/u;
	var AUTHOR_PROFILE_PATH = /^\/people\/[^/]+\/?$/u;
	var START_DISCUSSION_PATH = /^\/(?:group\/\d+\/?|register\/?)$/u;
	...
	var toSafeDoubanUrl = (rawHref) => {
		try {
			...
			if (url.protocol !== "http:" && url.protocol !== "https:" || url.hostname !== "www.douban.com" || url.port || url.username || url.password) return;
			return url;
		} catch {
			return;
		}
	...
	var normalizeTopicHref = (rawHref) => {
		...
		if (!url || !GROUP_TOPIC_PATH.test(url.pathname)) return "";
	...
	var extractReplies = (repliesCell) => {
		...
		const replies = match?.groups ? Number(match.groups.replies) : NaN;
		return Number.isSafeInteger(replies) ? replies : void 0;
	};
	...
	var extractStartDiscussionHref = (doc) => {
		const rawHref = doc.querySelector(".section-discussion .hd-ops .comment_btn")?.getAttribute("href")?.trim() ?? "";
		return rawHref ? normalizeDoubanHref(rawHref, START_DISCUSSION_PATH) : "";
	};
	var extractAllDiscussions = (doc) => {
		const link = doc.querySelector(".section-discussion table.olt ~ p a");
		const rawHref = link?.getAttribute("href")?.trim() ?? "";
		const href = rawHref ? normalizeDoubanHref(rawHref, GROUP_COLLECTION_PATH) : "";
		if (!href) return;
	...
	var extractDiscussions = (doc) => {
		const topics = [];
		for (const row of $$(".section-discussion table.olt tr", doc)) {
