...
(function() {
  ...
  title(id: $id) {
	...
	var parseData = (raw) => {
		...
		return {
			audienceCount: data.audienceScore?.reviewCount ?? 0,
			audienceScore,
	...
	var parseMediaScorecard = (html) => {
		...
		try {
			return parseData(mc.groups.json);
		} catch {
			return null;
		}
	};
	var parseRtPage = (html) => {
		...
		if (match?.groups?.json) {
			try {
				return parseData(match.groups.json);
			} catch {}
			return null;
		}
		return parseMediaScorecard(html);
	};
	...
	var buildRtRating = (rating) => {
		...
		labelRow.append(buildLabel(rating.criticsScore, ICON_TOMATO, "影评人", "is-critics"), el("div", { className: "atv-rt-label-sep" }), buildLabel(rating.audienceScore, ICON_POPCORN, "观众", "is-audience"));
		...
		section.append(el("div", {
			className: "atv-rating-panel-count",
			text: `影评人 ${criticsCountStr} · 观众 ${audienceCountStr}`
		}));
		return section;
	};
