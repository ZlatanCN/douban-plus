...
`;
	...
	var CommentVoteButton = ({ canVote, cid, className, count, onVote, voted }) => {
		...
		const vote = async () => {
			if (loading || state.voted || !cid) return;
		...
		return u$1("button", {
			...
			type: "button",
			children: ["有用 ", state.count]
		});
	};
	var CommentCard = ({ canVote, comment, onOpen, onVote }) => u$1("div", {
	...
	var CommentModalContent = ({ canVote, comment, onVote }) => {
	...
	var CommentModal = ({ canVote, comment, onClose, onVote }) => u$1(ModalShell, {
	...
	var CommentsSection = ({ canVote, comments, onOpen, onVote, subjectId }) => {
  ...
  title(id: $id) {
	...
	var StreamingSection = ({ streaming }) => streaming.length ? u$1(Section, {
		...
		children: u$1("div", {
			...
			children: streaming.map((item) => u$1("a", {
				class: "atv-stream-card",
				href: item.href,
				rel: "noopener",
				target: "_blank",
				children: [u$1("span", { children: item.name }), u$1("span", {
					class: "atv-stream-arrow",
					children: u$1(IconArrow, {})
				})]
			}, item.href))
		})
	}) : null;
	...
	var ReviewVoteButtons = ({ canVote, onVote, review, size = "normal" }) => {
		const [counts, setCounts] = d({
			useful: review.usefulCount ?? 0,
			useless: review.uselessCount ?? 0
		});
		const [voted, setVoted] = d(null);
		const [loading, setLoading] = d(false);
		const vote = async (type) => {
			if (loading || voted === type || !onVote) return;
			if (canVote && !canVote()) return;
			const previous = {
				counts,
				voted
			...
			if (result.ok) setCounts((current) => ({
		...
		return u$1(S, { children: [u$1("button", {
			...
			type: "button",
			children: ["没用 ", counts.useless]
		})] });
	};
	var ReviewCard = ({ canVote, onOpen, onVote, review }) => {
	...
	var ReviewModalContent = ({ canVote, onVote, review }) => {
		const handleClose = useModalClose();
	...
	var ReviewModal = ({ canVote, onClose, onVote, review }) => u$1(ModalShell, {
	...
	var ReviewsSection = ({ canVote, isTV, onOpen, onVote, reviews, subjectId }) => {
