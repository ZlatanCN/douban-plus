...
`;
	...
	var detectS3State = (root) => {
		...
		const commentEl = root.querySelector(".j.a_stars > span:not(.mr10):not(#rating):not(.color_gray)");
		let comment = "";
		let usefulCount = "";
		if (commentEl) {
	...
	var extractInterestTags = (root) => {
		const text = root.querySelector(".color_gray")?.textContent || "";
		const match = /^标签\s*[:：]\s*(?<tags>.+)$/u.exec(text.trim());
		if (!match?.groups?.tags) return [];
	...
	var StarRatingInput = ({ disabled = false, onChange, rating }) => u("fieldset", {
		...
		children: [
			...
			u("div", {
				...
				children: [u("span", { children: "我的评分" }), u("div", { children: [u("span", { children: "可选" }), rating > 0 ? u("button", {
					"aria-label": "清除评分",
			...
			u("div", {
				...
				children: Array.from({ length: 5 }, (_, index) => {
					...
					return u("button", {
						"aria-label": rating === value ? `取消 ${value} 星评分` : `评分 ${value} 星`,
						class: `atv-interest-modal-star${full ? " is-full" : ""}`,
						disabled,
						onClick: () => {
							if (!disabled) onChange(rating === value ? 0 : value);
						},
						type: "button",
						children: u("span", {
							"aria-hidden": "true",
	...
	var useInterestMarking = ({ adapters, loggedIn, onLoginRequired, subjectId, subjectTitle }) => {
