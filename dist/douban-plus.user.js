...
(function() {
	...
	var imdbCache = createCache("dp:imdb-cache");
  ...
  title(id: $id) {
	...
	var rottenCache = createCache("dp:rotten-cache");
	var toSlug = (title) => {
	...
	var openInterestModal = (state, callbacks) => {
		...
		const dismiss = () => {
			if (!content.overlay.isConnected) return;
		...
		content.overlay.addEventListener("click", (e) => {
			if (e.target === content.overlay) dismiss();
		});
		const keyHandler = (e) => {
			if (e.key === "Escape") {
				dismiss();
