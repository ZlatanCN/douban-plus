...
`;
	...
	var DiscussionsSection = ({ discussions }) => discussions.topics.length ? u(Section, {
		...
		children: u("div", {
			...
			children: discussions.topics.map((topic) => u("article", {
  ...
  title(id: $id) {
	...
	var normalizeTopicHref = (rawHref, baseUrl) => {
		try {
			const url = new URL(rawHref, baseUrl);
			if (url.protocol !== "http:" && url.protocol !== "https:") return "";
			url.searchParams.delete("_spm_id");
			return url.href;
		} catch {
			return "";
		}
	...
	var extractDiscussions = (doc) => {
		...
		for (const row of $$(".section-discussion table.olt tr", doc)) {
			...
			const href = rawHref ? normalizeTopicHref(rawHref, doc.baseURI) : "";
			if (title && href) {
				topics.push({
					href,
					title
